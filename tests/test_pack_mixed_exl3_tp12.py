...
from kquant.mixed_exl3 import LEGACY_LOGICAL_SCHEMA, SCHEMA
import scripts.pack_mixed_exl3_tp12 as pack_script
from scripts.pack_mixed_exl3_tp12 import (
    _ScheduleJob,
...
def test_new_candidate_manifest_freezes_explicit_logical_schema(tmp_path) -> None:
    args = _args(tmp_path)
    path = tmp_path / "mixed_exl3_candidate_manifest.json"
    ...
    assert actual["codebook"] == "mcg"
    assert actual["layout"] == "importance_ordered"
    assert json.loads(path.read_text()) == actual


def test_fieldless_candidate_manifest_resumes_as_historical_v2(tmp_path) -> None:
    args = _args(tmp_path)
    path = tmp_path / "mixed_exl3_candidate_manifest.json"
    historical = _manifest(args)
    historical.pop("logical_trellis_schema")
    path.write_text(json.dumps(historical))

    actual = _bind_candidate_manifest(args, path, create=False)

    assert actual == historical
    assert args.logical_trellis_schema == LEGACY_LOGICAL_SCHEMA
    assert "logical_trellis_schema" not in json.loads(path.read_text())
...
def test_pre_codebook_manifest_resumes_only_as_mcg(tmp_path) -> None:
    args = _args(tmp_path)
    path = tmp_path / "mixed_exl3_candidate_manifest.json"
    historical = _manifest(args)
    historical.pop("codebook")
    path.write_text(json.dumps(historical))

    assert _bind_candidate_manifest(args, path, create=False) == historical

    args.codebook = "mul1-e4m3"
    with pytest.raises(ValueError, match="does not match"):
        _bind_candidate_manifest(args, path, create=False)
...
def test_split_layer_merge_publishes_one_canonical_atomic_layer(
    ...
    for job in jobs:
        ...
        save_file(
            ...
            metadata={"codebook": "mul1-e4m3", "tailbite_context": "128"},
        )
        save_file(
