...
import kquant.pack.mixed_allocation as mixed_allocation
...
def _metrics() -> dict[str, torch.Tensor]:
    ...
    return {
        ...
        "compressed_damage": damage.clone(),
    }
...
def test_payload_metadata_preserves_legacy_contextless_contract() -> None:
    assert "tailbite_context" not in _expected_payload_metadata(
        7,
        codebook="mcg",
        tailbite_context=None,
    )
...
def test_selection_json_preserves_declared_logical_candidate_schema() -> None:
    metrics = _metrics()
    metrics["confirmation_improvement"] = torch.tensor(
        [0.5, float("nan"), 0.25], dtype=torch.float64
    )
    ...
    assert validate_selection_ledger_evidence(
        ...
        mode_ids=(0, 1),
        expert_ids=(0, 1, 2),
        logical_trellis_schema=LEGACY_LOGICAL_SCHEMA,
    ) == LEGACY_LOGICAL_SCHEMA
...
def test_candidate_pool_completion_binds_every_atomic_triplet(
    ...
    monkeypatch.setattr(mixed_allocation.C, "MOE_LAYERS", (1,))
    monkeypatch.setattr(mixed_allocation.C, "NUM_MOE_LAYERS", 1)
    (tmp_path / "candidates").mkdir()
    (tmp_path / "mixed_exl3_candidate_manifest.json").write_text("{}")
    stem = tmp_path / "candidates" / "mixed-exl3-layer-00001"
    ...
    legacy_document = candidate_pool_completion_document(
        tmp_path,
        jobs=2,
        logical_trellis_schema=LEGACY_LOGICAL_SCHEMA,
    )
    assert document["logical_trellis_schema"] == SCHEMA
    assert legacy_document["logical_trellis_schema"] == LEGACY_LOGICAL_SCHEMA
    assert legacy_document["content_sha256"] != document["content_sha256"]
...
def test_count_allocation_keeps_exact_global_top_damage_with_stable_ties() -> None:
    ...
    allocation = choose_mixed_allocation(damage, keep_count=3)

    assert np.argwhere(allocation.keep_mask).tolist() == [[0, 5], [2, 3], [4, 8]]
    assert allocation.retained_experts == 3
    assert allocation.container_bytes == total_container_bytes(allocation.keep_mask)
    assert allocation_optimality(allocation) == CERTIFIED_ALLOCATION_OPTIMALITY
...
def test_exact_byte_target_repairs_layer_alignment_at_minimum_local_loss() -> None:
    ...
    assert (
        allocation_optimality(allocation)
        == UNCERTIFIED_ALLOCATION_OPTIMALITY
    )
...
def test_allocation_document_carries_final_format_codes_and_damage_ledger() -> None:
    ...
    allocation = choose_mixed_allocation(damage, keep_count=1)
    ...
    assert (
        document["meta"]["allocation_optimality"]
        == CERTIFIED_ALLOCATION_OPTIMALITY
    )
...
def test_allocation_document_identifies_external_damage_source() -> None:
    ...
    document = allocation_document(
        pool, choose_mixed_allocation(damage, keep_count=0)
    )
