#!/usr/bin/env python3
"""Score selected TP12 mixed-EXL candidates on the untouched capture.
...
from kquant.mixed_candidates import request_documents, select_expert_rows
...
NUM_GPUS = 12
MANIFEST_NAME = "mixed_exl3_validation_manifest.json"
COMPLETION_NAME = "mixed_exl3_validation_complete.json"
...
def _manifest(args: argparse.Namespace) -> dict:
    candidate_manifest_path = args.candidate_pool / "mixed_exl3_candidate_manifest.json"
    ...
    codebook = candidate_manifest.get("codebook", CODEBOOK_MCG)
...
def worker(args: argparse.Namespace) -> None:
    ...
    _provenance, documents = _validate_corpus_contract(
        _read_json(args.candidate_pool / "mixed_exl3_candidate_manifest.json"),
        validation_capture=args.validation_capture,
        validation_report_path=args.validation_report,
    )
    resources = _WorkerResources(
...
def parent(args: argparse.Namespace) -> None:
    ...
    load_mixed_candidate_pool(args.candidate_pool)
    manifest = _manifest(args)
