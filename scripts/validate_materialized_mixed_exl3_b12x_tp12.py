...
def _b12x_codebook_contract(codebook: str) -> tuple[str, dict[str, int]]:
    ...
    if codebook == CODEBOOK_MCG:
        return "exl3_trellis_mcg", {"trellis_mcg": MCG_MULT}
    if codebook == CODEBOOK_MUL1_E4M3:
        return "exl3_trellis_mul1_e4m3", {
            "trellis_mul1_e4m3": MUL1_MULT
        }
    if codebook == CODEBOOK_SQG_NORMAL_E4M3:
        return "exl3_trellis_sqg_e4m3", {}
    raise ValueError(f"unsupported materialized EXL3 codebook: {codebook!r}")
    ...
    for slot in _sample_slots(len(serving_ids)):
        ...
        for matrix in ("w1", "w3", "w2"):
            if x4_reader is None:
                source = reader.read_kept_matrix(expert, matrix)
            else:
                record = x4_reader.read(expert, matrix)
                source = PackedMXFP4Matrix(
                    packed=record.packed,
                    scale=record.scale,
                )
            packed, scale = mxfp4_rank_matrix_components(
                source, matrix, rank
            )
            if matrix == "w1":
...
def _load_vllm_reader() -> tuple[Any, Path]:
    try:
        module = importlib.import_module(
            "vllm.model_executor.layers.quantization.kquant_mixed_exl3_tp12"
        )
    except ImportError as exc:
        raise RuntimeError(
            "the production vLLM mixed-EXL reader is unavailable; run with "
...
def _resolve_x4_path(args: argparse.Namespace) -> Path | None:
    if args.x4_file is not None:
        return args.x4_file.resolve()
    if args.artifact is None:
        return None
    candidate = x4_layer_path(args.artifact, args.layer).resolve()
...
def _regularized_matrix(
    ...
    *,
    ...
    regularized = decode_mixed_regularized_weight(
...
def _select_route_slots(payload: TP12CompressedRankPayload, topk: int) -> torch.Tensor:
    """Prefer P24-bearing experts so a sparse mixed layer exercises both arms."""

    mixed = (payload.fc1_pair_modes != 0) | (payload.fc2_pair_modes != 0)
    mixed_slots = torch.nonzero(mixed).flatten()
    uniform_slots = torch.nonzero(~mixed).flatten()
    slots = torch.cat((mixed_slots, uniform_slots))[:topk]
...
def validate(args: argparse.Namespace) -> dict[str, Any]:
    ...
    with TP12LayerReader(layer_path) as reader:
        ...
        external_x4 = (
            reader.header.layout.keep_storage == KEEP_STORAGE_EXTERNAL_X4
        )
        if external_x4 != (x4_path is not None):
            requirement = "requires" if external_x4 else "must not use"
            raise ValueError(
                f"materialized slab {requirement} an external X4 sidecar"
            )
        x4_reader = X4LayerReader(x4_path) if x4_path is not None else None
        if x4_reader is not None and x4_reader.layer != args.layer:
    ...
    return {
        "kind": "kquant_materialized_mixed_exl3_b12x_tp12_closure",
        "layer_file": str(layer_path),
        "layer_file_bytes": layer_path.stat().st_size,
        "x4_file": None if x4_path is None else str(x4_path),
        "x4_file_bytes": None if x4_path is None else x4_path.stat().st_size,
        "x4_file_sha256": None if x4_path is None else _sha256(x4_path),
        "layer": args.layer,
        "rank": args.rank,
        "tp_size": TP_SIZE,
...
def main() -> None:
    ...
    parser.add_argument(
        "--x4-file",
        type=Path,
        help="external X4 peer required with a standalone SQRT-C --layer-file",
    )
