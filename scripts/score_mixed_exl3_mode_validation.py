...
for every assignment whose fit/confirmation selector accepted mixed rate on
...
from kquant.pack.mixed_mode_validation import (
    ...
    load_mixed_mode_validation_scores,
)
...
def _sha256_files(paths: tuple[Path, ...]) -> str:
    ...
    digest = hashlib.sha256()
    digest.update(b"kquant-exl3-quantizer-sources-v1\0")
    for path in paths:
...
def _selected_validation_metrics_path(root: Path, layer: int) -> Path:
    return (
        root
        / "scores"
        / f"mixed-exl3-layer-{layer:05d}.validation.safetensors"
    )
...
def _load_quantizer_module(root: Path):
    ...
    return __import__(quantize_exl3.__module__, fromlist=["*"])
...
def _manifest(
    ...
    pool: object,
    validation: MixedValidationScores,
) -> dict:
    ...
    quantizer_sources = (
        exllamav3_root / "exllamav3/modules/quant/exl3_lib/quantize.py",
        exllamav3_root / "exllamav3/exllamav3_ext/quant/codebook.cuh",
        exllamav3_root
        / "exllamav3/exllamav3_ext/quant/quantize_tiles_kernel.cuh",
        exllamav3_root / "exllamav3/exllamav3_ext/quant/quantize.cu",
    )
    provenance_files = {
...
def _verify_training_r0(
    candidate: MixedCandidateEncoding,
...
def _payload_digest(candidate: MixedCandidateEncoding) -> str:
    digest = hashlib.sha256()
...
def score_layer(
    ...
    *,
    ...
    print(
        ...
        f"{len(audited_experts)} selected mixed experts in {ledger['seconds']:.0f}s",
        flush=True,
    )
    return ledger
...
def parent(args: argparse.Namespace) -> None:
    ...
    pool = load_mixed_candidate_pool(args.candidate_pool)
    ...
    if completion_path.exists():
        result = load_mixed_mode_validation_scores(
            args.dest,
            pool,
            validation,
        )
