#!/usr/bin/env python3
"""Build the TP12 phase-1 all-expert mixed-EXL candidate pool.
...
from kquant.pack.mixed_exl3 import MIXED_SEARCH_LAYOUTS
...
def _layer_stem(layer: int, experts: list[int]) -> str:
    if experts == list(range(C.NUM_EXPERTS)):
        return f"mixed-exl3-layer-{layer:05d}"
    digest = __import__("hashlib").blake2b(
        ",".join(map(str, experts)).encode(), digest_size=6
    ).hexdigest()
    return f"mixed-exl3-layer-{layer:05d}-subset-{digest}"
...
def _load_quantizer_module(root: Path, codebook: str = CODEBOOK_MCG):
    ...
    quantize_exl3 = _load_exl3_quantizer(root)
...
def _allocate_metrics(
    ...
    return {
        ...
        # Compatibility alias retained while the phase-1 tooling is in flight.
        "compressed_damage": torch.zeros(count, dtype=torch.float64),
    }
...
def _store_metrics(
    ...
    metrics["compressed_damage"][row] = official_source_excess_sse
    ...
    *,
    ...
    ledger = {
        ...
        "damage_contract": {
            ...
            "candidate": "selected coupled w1/w3/w2 mixed-EXL reconstruction",
...
def _manifest(args: argparse.Namespace) -> dict:
    return {
        ...
        "logical_trellis_schema": args.logical_trellis_schema,
        "codebook": args.codebook,
        "layout": getattr(args, "layout", "importance_ordered"),
...
def _bind_candidate_manifest(
    ...
    *,
    ...
    """Freeze one logical schema, including pre-field v3 pool manifests.

    Candidate-pool schema version 3 initially omitted this manifest field and
    its encoder emitted proto-v2 logical descriptors.  Missing therefore has
    one unambiguous historical meaning.  Preserve that contract on resume so
    a source update cannot mix v2 and v3 descriptor labels across layers.
    """
    ...
    # Pre-codebook manifests always mean MCG.  Similarly, the oldest v3 pool
    # manifests omitted the logical descriptor field and therefore mean the
    # legacy proto-v2 descriptor.  Accept every unambiguous combination only
    # when the requested run has those exact historical semantics.
    for omit_tailbite_context in (False, True):
        if omit_tailbite_context and int(getattr(args, "tailbite_context", 128)) != 128:
            continue
        for omit_codebook in (False, True):
            if omit_codebook and args.codebook != CODEBOOK_MCG:
                continue
            for omit_layout in (False, True):
                if omit_layout and getattr(args, "layout", "importance_ordered") != "importance_ordered":
                    continue
                for omit_tf32 in (False, True):
                    if omit_tf32 and bool(getattr(args, "ldlq_tf32", False)):
                        continue
                    for omit_logical_schema in (False, True):
                        legacy_expected = dict(expected)
                        if omit_tailbite_context:
                            legacy_expected.pop("tailbite_context")
                        if omit_codebook:
                            legacy_expected.pop("codebook")
                        if omit_layout:
                            legacy_expected.pop("layout")
                        if omit_tf32:
                            legacy_expected.pop("ldlq_tf32")
                        if omit_logical_schema:
                            legacy_expected.pop("logical_trellis_schema")
                        if actual == legacy_expected:
                            if omit_logical_schema:
                                args.logical_trellis_schema = LEGACY_LOGICAL_SCHEMA
                            return actual
...
def parent(args: argparse.Namespace) -> None:
    ...
    manifest_path = args.dest / "mixed_exl3_candidate_manifest.json"
    ...
    subprocess.run(
        [
            sys.executable,
            str(Path(__file__).resolve().with_name("finalize_mixed_exl3_pool.py")),
            str(args.dest),
            "--jobs",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--codebook",
        choices=EXL3_CODEBOOKS,
        default=CODEBOOK_MCG,
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "tile boundary (1..128; 128 is the legacy exact search)"
        ),
    )
    ...
    parser.add_argument(
        ...
        default=MIXED_EXL3_SCHEMA,
        help=(
            "logical candidate descriptor schema; frozen in new manifests and "
            "inferred as proto-v2 for historical field-less manifests"
        ),
    )
