...
from kquant.pack.mixed_validation import load_mixed_validation_scores


SUMMARY_KIND = "kquant_tp12_mixed_exl3_validation_summary"
...
def compare_damage_rankings(
    ...
    *,
    ...
    selection_allocation = choose_mixed_allocation(
        selection_damage, target_container_bytes=target_container_bytes
    )
    validation_allocation = choose_mixed_allocation(
    ...
    return {
        ...
        "keep_set": {
            ...
            "selection_optimality": allocation_optimality(
                selection_allocation
            ),
            "validation_optimality": allocation_optimality(
...
def bootstrap_keep_stability(
    ...
    *,
    ...
    base_allocation = choose_mixed_allocation(
        full_damage,
    ...
    for replicate in range(replicates):
        ...
        sampled_allocation = choose_mixed_allocation(
            sampled_damage,
            target_container_bytes=target_container_bytes,
        )
        sampled = sampled_allocation.keep_mask
        uncertified_replicates += int(
            allocation_optimality(sampled_allocation)
            != CERTIFIED_ALLOCATION_OPTIMALITY
        )
    ...
    return {
        ...
        "base_allocation_optimality": allocation_optimality(
            base_allocation
        ),
