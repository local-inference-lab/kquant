...
from scripts.export_mixed_exl3_tp12_benchmark_fixture import export_fixture
from scripts.summarize_mixed_exl3_tp12_runtime_mix import summarize_runtime_mix


KIND = "kquant_mixed_exl3_tp12_performance_run"
SCHEMA_VERSION = 1
TP_SIZE = 12
...
def main() -> None:
    ...
    candidate_manifest = json.loads(
        (candidate_pool / "mixed_exl3_candidate_manifest.json").read_text()
    )
    codebook = candidate_manifest.get("codebook", "mcg")
    if codebook not in {"mcg", "sqg-normal-e4m3"}:
        raise ValueError(
