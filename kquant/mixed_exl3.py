...
the full 16-bit cyclic trellis states consumed by the MCG decoder.
"""
...
LEGACY_LOGICAL_SCHEMA = "kquant_mixed_exl3_tp12_proto_v2"
LOGICAL_CANDIDATE_SCHEMAS = (LEGACY_LOGICAL_SCHEMA, SCHEMA)
...
# for w2.  Each value is in 0..12.  0xff denotes retained MXFP4.  This
# deliberately represents the complete experiment space even though the
# frozen initial encoder uses one shared mode from a smaller alphabet.  The
# wider field leaves room for diagnostic candidates without changing P24/P33.
FORMAT_BITS = 8
FORMAT_MXFP4 = 0xFF
...
# The logical trellis payload remains proto-v3, but the materialized slab
# header is versioned independently.  V4 makes the reconstruction family
# self-identifying; V3 implicitly meant MCG and is retained read-only.  V5 is
# the SQRT-C slab: the exact MXFP4 runtime tier is stored externally as X4,
# while this file contains only trellis experts and their transform metadata.
LEGACY_LAYER_MAGIC = b"KQTP12V3"
LEGACY_LAYER_HEADER_VERSION = 3
_LEGACY_LAYER_HEADER_STRUCT = struct.Struct("<8s8H2I5Q")
LAYER_MAGIC = b"KQTP12V4"
LAYER_HEADER_VERSION = 4
_LAYER_HEADER_STRUCT = struct.Struct("<8s9H2I5Q")
SQRT_C_LAYER_MAGIC = b"KQTP12V5"
SQRT_C_LAYER_HEADER_VERSION = 5
_SQRT_C_LAYER_HEADER_STRUCT = struct.Struct("<8s10H2I5Q")
KEEP_STORAGE_INLINE_MXFP4 = "inline-mxfp4"
KEEP_STORAGE_EXTERNAL_X4 = "external-x4"
KEEP_STORAGE_IDS = {
    KEEP_STORAGE_INLINE_MXFP4: 0,
    KEEP_STORAGE_EXTERNAL_X4: 1,
}
...
class ModeSpec:
    ...
    ``context_bits`` may use coarse legacy contexts or one context per final
    128-channel record.  ``mode_id`` is always the number of K2/K4 record
    exchanges, so schedules with identical physical rates share an ID.
    """
    ...
    def __post_init__(self) -> None:
        ...
        if not 0 <= self.mode_id <= PAIRS_PER_EXPERT:
            raise ValueError(f"mode_id must be in 0..{PAIRS_PER_EXPERT}")
        if not self.name:
...
RATE_TRANSFER_MODES = tuple(
    _rate_transfer_mode(r) for r in range(PAIRS_PER_EXPERT + 1)
)
R0 = RATE_TRANSFER_MODES[0]
R3 = RATE_TRANSFER_MODES[3]
R6 = RATE_TRANSFER_MODES[6]
R12 = RATE_TRANSFER_MODES[12]

# Compatibility specs reproduce the context granularity used by the pilot.
# New studies and manifests use the canonical R names above.
U4 = ModeSpec(0, "U4", (3, 3, 3, 3))
M4 = ModeSpec(6, "M4", (2, 3, 3, 4))
T8 = ModeSpec(3, "T8", (2, 3, 3, 3, 3, 3, 3, 4))
LEGACY_MODE_ALIASES = (U4, M4, T8)
...
_MODES_BY_NAME = {
    **{mode.name: mode for mode in RATE_TRANSFER_MODES},
    **{mode.name: mode for mode in LEGACY_MODE_ALIASES},
}
...
class ExpertFormatSpec:
    ...
    def __post_init__(self) -> None:
        ...
        for name, value in (("r13", self.r13), ("r2", self.r2)):
            ...
            if not 0 <= value <= PAIRS_PER_EXPERT:
                raise ValueError(f"{name} must be in 0..{PAIRS_PER_EXPERT}")
    ...
    def from_code(cls, code: int) -> "ExpertFormatSpec":
        ...
        if r13 > PAIRS_PER_EXPERT or r2 > PAIRS_PER_EXPERT:
...
class TP12LayerLayout:
    ...
    scale vectors, then twelve identical rank sections.  Each rank section is
    compressed-expert-major trellis pairs, local scale triplets, and retained
    MXFP4 experts.  Expert slots are ascending expert ID within their tier and
    are therefore derivable from the format table without stored offsets.
    """
    ...
    def __post_init__(self) -> None:
        ...
        if self.keep_storage not in KEEP_STORAGE_IDS:
            raise ValueError(f"unsupported high-tier storage: {self.keep_storage!r}")
    ...
    def rank_keep_bytes(self) -> int:
        if self.keep_storage == KEEP_STORAGE_EXTERNAL_X4:
            return 0
        return self.kept_experts * EXPERT_RANK_MXFP4_BYTES

    @property
    def rank_keep_resident_bytes(self) -> int:
        """Decoded W4A16 bytes resident on one rank after artifact loading."""
    ...
    @property
    def resident_bytes_per_rank(self) -> int:
        # A 4-KiB hot metadata page stores uint8 format codes and uint16 tier
        # slots. X4 is decoded into the same W4A16 resident representation as
        # inline MXFP4, even though those bytes are absent from the trellis slab.
        return (
            STORAGE_ALIGNMENT
            + SHARED_SCALE_SECTION_BYTES
            + self.rank_trellis_bytes
            + self.rank_scale_section_bytes
            + self.rank_keep_resident_bytes
        )

    @property
    def resident_bytes_fleet(self) -> int:
        return TP_SIZE * self.resident_bytes_per_rank
    ...
    def rank_keep_offset(self, rank: int) -> int:
        return self.rank_scale_offset(rank) + self.rank_scale_section_bytes
    ...
    def kept_matrix_offset(self, rank: int, kept_slot: int, matrix: str) -> int:
        if self.keep_storage != KEEP_STORAGE_INLINE_MXFP4:
            raise ValueError("external X4 matrices have no inline slab offset")
        self._validate_slot(kept_slot, self.kept_experts, "kept_slot")
        try:
            matrix_index = MATRIX_INDEX[matrix]
        except KeyError as exc:
            raise ValueError(f"unknown expert matrix: {matrix}") from exc
        return (
            self.rank_keep_offset(rank)
            + kept_slot * EXPERT_RANK_MXFP4_BYTES
            + matrix_index * RANK_MXFP4_MATRIX_BYTES
        )
    ...
    def to_manifest(self) -> dict[str, int | str]:
        return {
            ...
            "rank_keep_resident_bytes": self.rank_keep_resident_bytes,
            ...
            "resident_bytes_per_rank": self.resident_bytes_per_rank,
            "resident_bytes_fleet": self.resident_bytes_fleet,
        }
...
class TP12LayerHeader:
    ...
    def to_bytes(self) -> bytes:
        if self.layout.keep_storage == KEEP_STORAGE_EXTERNAL_X4:
            prefix = _SQRT_C_LAYER_HEADER_STRUCT.pack(
                SQRT_C_LAYER_MAGIC,
            ...
            prefix = _LAYER_HEADER_STRUCT.pack(
                LAYER_MAGIC,
                LAYER_HEADER_VERSION,
                LAYER_HEADER_BYTES,
                TP_SIZE,
                self.layer,
                EXPERTS_PER_LAYER,
    ...
    def from_bytes(cls, payload: bytes | bytearray | memoryview) -> "TP12LayerHeader":
        ...
        magic = raw[:8]
        if magic == SQRT_C_LAYER_MAGIC:
            fields = _SQRT_C_LAYER_HEADER_STRUCT.unpack_from(raw)
            (
                magic,
                version,
                header_bytes,
            ...
            if keep_storage != KEEP_STORAGE_EXTERNAL_X4:
                raise ValueError("V5 layer header must use external X4 storage")
            expected_version = SQRT_C_LAYER_HEADER_VERSION
            expected_multiplier = CODEBOOK_MULTIPLIERS[codebook]
            padding_begin = _SQRT_C_LAYER_HEADER_STRUCT.size
        elif magic == LAYER_MAGIC:
            fields = _LAYER_HEADER_STRUCT.unpack_from(raw)
            (
                magic,
                version,
                header_bytes,
            ...
            expected_version = LAYER_HEADER_VERSION
            expected_multiplier = CODEBOOK_MULTIPLIERS[codebook]
            keep_storage = KEEP_STORAGE_INLINE_MXFP4
            padding_begin = _LAYER_HEADER_STRUCT.size
        elif magic == LEGACY_LAYER_MAGIC:
            fields = _LEGACY_LAYER_HEADER_STRUCT.unpack_from(raw)
            (
                magic,
                version,
                header_bytes,
            ...
            expected_version = LEGACY_LAYER_HEADER_VERSION
            expected_multiplier = MCG_MULT
            keep_storage = KEEP_STORAGE_INLINE_MXFP4
            padding_begin = _LEGACY_LAYER_HEADER_STRUCT.size
        else:
            raise ValueError(
                f"layer header magic mismatch: {magic!r} != {LAYER_MAGIC!r}"
            )
        expected_scalars = {
    ...
    def to_manifest(self) -> dict[str, int | str]:
        if self.layout.keep_storage == KEEP_STORAGE_EXTERNAL_X4:
            magic = SQRT_C_LAYER_MAGIC
            version = SQRT_C_LAYER_HEADER_VERSION
        else:
            magic = LAYER_MAGIC
            version = LAYER_HEADER_VERSION
        return {
            "layer": self.layer,
...
def resolve_mode(mode: ModeSpec | str | int) -> ModeSpec:
    ...
    if isinstance(mode, str):
        try:
            ...
            raise ValueError(f"unknown mixed-EXL mode: {mode}") from exc
    if isinstance(mode, int) and not isinstance(mode, bool):
        try:
            return _MODES_BY_ID[mode]
        except KeyError as exc:
            raise ValueError(f"unknown mixed-EXL mode ID: {mode}") from exc
...
def _validate_k(bits: int) -> None:
    if isinstance(bits, bool) or not isinstance(bits, int) or bits not in (2, 3, 4):
        raise ValueError("TP12 mixed EXL supports K=2, K=3, or K=4")
...
class TP12TrellisDescriptor:
    ...
    def __post_init__(self) -> None:
        # Proto-v3 changed only the outer rank-slab ownership mapping.  The
        # allocation-independent candidate stream remains in logical pair
        # order and is byte-identical to proto-v2, so a source candidate may
        # retain either explicit label.  Materialized layer headers remain
        # unconditionally v3.
        if self.schema not in LOGICAL_CANDIDATE_SCHEMAS:
            raise ValueError(f"unsupported mixed-EXL schema: {self.schema}")
...
class PackedTP12Trellis:
    ...
    def validate(self) -> None:
        if self.payload.dtype != torch.int16:
            raise TypeError("mixed trellis payload must use torch.int16")
        if self.payload.ndim != 1:
            raise ValueError("mixed trellis payload must be one-dimensional")
        if self.payload.numel() != self.descriptor.payload_words:
            raise ValueError(
                "mixed trellis payload length does not match its descriptor"
            )
...
def unpack_tp12_trellis_edges(packed: PackedTP12Trellis) -> torch.Tensor:
    ...
    if cursor != packed.payload.numel():
        raise ValueError("mixed trellis payload has trailing data")
    return output
...
def decode_tp12_exl3_weight(
    ...
    *,
    ...
    """Decode a mixed TP12 payload to its physical EXL-oriented weight."""
    ...
    return decode_mixed_exl3_weight(
