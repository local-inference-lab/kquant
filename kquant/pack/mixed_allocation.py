"""Validation and global allocation for TP12 mixed-EXL candidate pools.
...
ALLOCATION_KIND = "kquant_tp12_mixed_exl3_allocation"
...
CANDIDATE_POOL_COMPLETION_KIND = "kquant_tp12_mixed_exl3_candidate_completion"
CANDIDATE_POOL_COMPLETION_SCHEMA_VERSION = 2
CANDIDATE_POOL_COMPLETION_FILENAME = "mixed_exl3_candidate_complete.json"


@dataclass(frozen=True)
class MixedCandidatePool:
    root: Path
    manifest: dict
    damage: np.ndarray
...
def _candidate_content_sha256(
    ...
    digest = hashlib.sha256()
    digest.update(b"kquant-mixed-candidate-pool-v2\0")
...
def _canonical_layer_paths(root: Path, layer: int) -> tuple[Path, Path, Path]:
    stem = f"mixed-exl3-layer-{layer:05d}"
...
def candidate_pool_completion_document(
    ...
    *,
    jobs: int = 1,
    logical_trellis_schema: str = MIXED_EXL3_SCHEMA,
) -> dict:
    ...
    manifest_path = root / "mixed_exl3_candidate_manifest.json"
    ...
    codebook = manifest.get("codebook", CODEBOOK_MCG)
...
def validate_candidate_pool_completion(
    ...
    *,
    ...
    expected_scalars = {
        ...
        "complete": True,
        "candidate_schema_version": CANDIDATE_POOL_SCHEMA_VERSION,
        "manifest": "mixed_exl3_candidate_manifest.json",
        "layer_count": C.NUM_MOE_LAYERS,
    }
    ...
    manifest = _read_json(manifest_path)
    manifest_codebook = manifest.get("codebook", CODEBOOK_MCG)
    completion_codebook = completion.get("codebook", CODEBOOK_MCG)
    if (
        manifest_codebook not in EXL3_CODEBOOKS
...
def validate_layer_metrics(
    ...
    *,
    ...
    if "compressed_damage" in metrics and not torch.equal(
        metrics["compressed_damage"], stored_damage
    ):
        raise ValueError(
            "legacy compressed_damage alias disagrees with the damage metric"
        )
    return {
...
def _expected_payload_metadata(
    ...
    *,
    ...
    if tailbite_context is not None:
        metadata["tailbite_context"] = str(tailbite_context)
    return metadata
...
def _validate_payload_header(
    ...
    *,
    codebook: str,
    tailbite_context: int | None = None,
) -> None:
    ...
    legacy_metadata.pop("codebook")
    if metadata != expected_metadata and not (
        codebook == CODEBOOK_MCG and metadata == legacy_metadata
    ):
        raise ValueError(
...
def load_mixed_candidate_pool(
    ...
    *,
    ...
    if (
        ...
        or any(not 0 <= mode <= 12 for mode in mode_ids)
    ...
    if tailbite_context is not None and (
    ...
    for layer in C.MOE_LAYERS:
        ...
        if ledger.get("codebook", CODEBOOK_MCG) != codebook:
            raise ValueError(f"layer {layer} codebook drifted")
        if tailbite_context is not None and (
            ledger.get("tailbite_context") != tailbite_context
        ):
...
def total_container_bytes(keep_mask: np.ndarray) -> int:
    ...
    return sum(
        TP12LayerLayout(
            compressed_experts=C.NUM_EXPERTS - int(keep_mask[row].sum()),
            kept_experts=int(keep_mask[row].sum()),
        ).disk_bytes
...
def keep_mask_from_allocation_document(document: dict) -> np.ndarray:
    """Parse either the current mixed allocation or the legacy keep/EXL3 map."""
...
def choose_mixed_allocation(
    ...
    *,
    ...
    total = damage.size
    all_compressed = sum(
        TP12LayerLayout(C.NUM_EXPERTS, 0).disk_bytes
        for _ in range(C.NUM_MOE_LAYERS)
    )
    layer_base = TP12LayerLayout(C.NUM_EXPERTS, 0).disk_bytes
    if any(
        TP12LayerLayout(C.NUM_EXPERTS - keep, keep).disk_bytes
    ...
    all_kept = sum(
        TP12LayerLayout(0, C.NUM_EXPERTS).disk_bytes
        for _ in range(C.NUM_MOE_LAYERS)
    )
    ...
    actual_bytes = total_container_bytes(keep_mask)
    repair_swaps = 0
    repair_damage = 0.0
    if target_container_bytes is not None and actual_bytes > target_container_bytes:
        while actual_bytes > target_container_bytes:
            ...
            for source in range(C.NUM_MOE_LAYERS):
                ...
                if source_count == 0:
                    continue
                source_before = TP12LayerLayout(
                    C.NUM_EXPERTS - source_count, source_count
                ).disk_bytes
                source_after = TP12LayerLayout(
                    C.NUM_EXPERTS - source_count + 1, source_count - 1
                ).disk_bytes
                ...
                for destination in range(C.NUM_MOE_LAYERS):
                    ...
                    if destination == source or destination_count == C.NUM_EXPERTS:
                        continue
                    destination_before = TP12LayerLayout(
                        C.NUM_EXPERTS - destination_count, destination_count
                    ).disk_bytes
                    destination_after = TP12LayerLayout(
                        C.NUM_EXPERTS - destination_count - 1,
                        destination_count + 1,
                    ).disk_bytes
                    byte_delta = (
                        source_after
        ...
        keep_mask = _prefix_mask(orders, counts)
        if total_container_bytes(keep_mask) != actual_bytes:
...
def allocation_optimality(allocation: MixedAllocation) -> str:
...
def allocation_document(
    pool: MixedCandidatePool, allocation: MixedAllocation
) -> dict:
    ...
    return {
        ...
        "meta": {
            ...
            "allocation_optimality": allocation_optimality(allocation),
