"""Untouched-corpus scoring for selected TP12 mixed-EXL candidates.
...
from kquant.mixed_candidates import RoutedRows, functional_sse_by_request
...
from kquant.pack.mixed_candidates import MatrixStore, candidate_tensor_name
from kquant.tp_simulator import situ


VALIDATION_SCORE_KIND = "kquant_tp12_mixed_exl3_validation_scores"
...
def load_mixed_validation_scores(
    ...
    try:
        ...
        raise TypeError("candidate_pool is not a MixedCandidatePool") from exc
    root = Path(root).resolve()
    manifest_path = root / "mixed_exl3_validation_manifest.json"
    ...
    candidate_manifest_path = candidate_root / "mixed_exl3_candidate_manifest.json"
    ...
    completion = _read_json(root / "mixed_exl3_validation_complete.json")
    if (
    ...
    content_digest.update(b"kquant-mixed-validation-score-set-v3\0")
    _update_content_digest(content_digest, root, manifest_path)
    _update_content_digest(
        content_digest,
        root,
        root / "mixed_exl3_validation_complete.json",
    )
    ...
    for row, layer in enumerate(C.MOE_LAYERS):
        stem = f"mixed-exl3-layer-{layer:05d}.validation"
